...
class GridCollectionViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout, UIGestureRecognizerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let lpgr : UILongPressGestureRecognizer = UILongPressGestureRecognizer(target: self, action: #selector(GridCollectionViewController.handleLongPress(_:)))
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        return CGSize(width: collectionView.bounds.size.height/CGFloat(numRows + 1) - 2, height: collectionView.bounds.size.height/CGFloat(numRows + 1) - 2)
    }
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if game[row][col].isMarked {
            ...
            if game[row][col].isMine {
                ...
                cell.displayNumber(surroundingMines: game[row][col].surroundingMinesCount)
            }
        } else {
            cell.displayHidden()
        }
    ...
    func countMines() {
        for row in 0..<numRows {
            for col in 0..<numColumns {
                ...
                print("(\(row), \(col)) has \(game[row][col].surroundingMinesCount) mines")
            }
        }
    }
}
